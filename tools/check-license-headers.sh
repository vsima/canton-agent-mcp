...
#
# Config and build files are not source and are exempt; node_modules is never
# checked. Pass --fix to prepend the header to any file missing it.
...
sources() {
  find "$ROOT/src" "$ROOT/test" -name '*.ts' 2>/dev/null
}

missing=0
while IFS= read -r file; do
  if ! head -4 "$file" | grep -q "SPDX-License-Identifier: Apache-2.0"; then
    if $fix; then
